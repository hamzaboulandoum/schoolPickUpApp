  ...
  while (c != 100) {
    ...
    for (var i = 0; i < b; i++) {
      ...
      for (int i = 0; i < r.sequence.length - 1; i++) {
        auxformones[r.sequence[i]][r.sequence[i + 1]] += q / (f.abs() + t);
        auxformones[r.sequence[i + 1]][r.sequence[i]] += q / (f.abs() + t);
      }
    }
...
Route generateRoute(Graph g, List bus) {
  var r = Route();
